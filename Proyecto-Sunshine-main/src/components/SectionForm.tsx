...
export default function SectionForm({ isOpen, onClose, onSave, editingSection, categoryId }: SectionFormProps) {
  ...
  const handleSelectAllCountries = () => {
    setFormData(prev => ({
      ...prev,
      countries: prev.countries.length === AVAILABLE_COUNTRIES.length 
        ? [] 
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div>
            ...
            {validationErrors.sectionName && (
              <p className="text-red-500 text-xs mt-1">{validationErrors.sectionName}</p>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                disabled={isLoading}
              >
                {formData.countries.length === AVAILABLE_COUNTRIES.length ? 'Deseleccionar todos' : 'Seleccionar todos'}
              </button>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pt-4 sm:pt-6 border-t border-gray-200 space-y-3 sm:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
              ...
              >
                ...
                <span>
                  {isLoading ? 'Guardando...' : (editingSection ? 'Actualizar' : 'Crear')} Sección
                </span>
              </button>
            </div>
          </div>
