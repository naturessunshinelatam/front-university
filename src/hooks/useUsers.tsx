...
export function useUsers() {
      ...
      if (response.ok) {
        ...
        const mappedUsers = Array.isArray(data)
          ? data.map((user: any) => ({
              ...user,
              accesibleCategories:
                user.categories || user.accesibleCategories || [],
            }))
          : [];
  ...
  const updateUser = async (
    userData: Omit<User, "createdAt" | "lastLogin"> & { password?: string },
  ) => {
    setIsLoading(true);
    setError(null);
  ...
  const deleteUser = async (email: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(
          errorData.message || errorData.error || "Error al eliminar usuario",
        );
        return false;
      }
    } catch (error) {
