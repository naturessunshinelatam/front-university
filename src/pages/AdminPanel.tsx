...
import { useTopPagesAndViews } from "../hooks/useAnalyticsData";
...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState(
    location.state?.activeTab || "content",
  );
  ...
  const { contentItems, systemConfig, updateSystemConfig, getContentStats } =
    useContentData();
  ...
  const {
    contents,
    fetchAllContent,
    deleteContent: deleteContentAPI,
    isLoading: isLoadingContent,
  } = useContent();

  // const topPages = useTopPagesAndViews();
  // console.log("top pages and views", topPages.data);
  ...
  const handleEditUser = (user: any) => {
    ...
    console.log(
      "🔍 handleEditUser - user.accesibleCategories:",
      user.accesibleCategories,
    );
    console.log(
      "🔍 handleEditUser - user.accesibleCategories type:",
      typeof user.accesibleCategories,
    );
    console.log(
      "🔍 handleEditUser - user.accesibleCategories isArray:",
      Array.isArray(user.accesibleCategories),
    );
          ...
          try {
            ...
            if (success) {
              ...
              showAlert({
                ...
                message:
                  "No se pudo eliminar el contenido. Por favor, inténtalo de nuevo.",
              });
            }
          } catch (error) {
            ...
            showAlert({
              ...
              message:
                "Ocurrió un error al conectar con el servidor. Por favor, inténtalo más tarde.",
            });
          }
        },
      });
    ...
    try {
      ...
      if (success) {
        ...
        showAlert({
          ...
          title: "❌ Error al Eliminar",
          message:
            "No se pudo eliminar el usuario. Por favor, verifica los permisos e inténtalo de nuevo.",
        });
      }
    } catch (error) {
        ...
        try {
          ...
          if (success) {
            ...
            showAlert({
              ...
              title: "❌ Error al Eliminar",
              message:
                "No se pudo eliminar la sección. Por favor, inténtalo de nuevo.",
            });
          }
        } catch (error) {
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg">
            <div className="text-center">
              <div className="text-2xl font-bold text-[#124C45]">
                {realContents.length}
              </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {
                  realContents.filter((item) => item.status === "publicado")
                    .length
                }
              </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-yellow-600">
                {
                  realContents.filter((item) => item.status === "borrador")
                    .length
                }
              </div>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[200px]">
                    Título
                  </th>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[120px]">
                    Categoría
                  </th>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[80px]">
                    Tipo
                  </th>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[100px] hidden lg:table-cell">
                    Autor
                  </th>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[100px] hidden xl:table-cell">
                    Países
                  </th>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[80px]">
                    Estado
                  </th>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[70px] hidden lg:table-cell">
                    Vistas
                  </th>
                  <th className="text-left py-3 px-2 sm:px-4 font-semibold text-gray-900 min-w-[120px] sticky right-0 bg-gray-50">
                    Acciones
                  </th>
                </tr>
              </thead>
                  ...
                  >
                    <td className="py-3 px-2 sm:px-4">
                      <div>
                        ...
                        {item.description && (
                          <p className="text-sm text-gray-500 line-clamp-2 max-w-[180px]">
                            {item.description}
                          </p>
                        )}
      ...
      <div className="space-y-4">
        <div className="text-center sm:text-left">
          ...
          <p className="text-gray-600">
            Administra la estructura de contenido de Universidad Sunshine
          </p>
        </div>
        <button
            ...
            <div className="p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h4 className="font-medium text-gray-900">
                  Secciones ({category.sections.length})
                </h4>
                <button
              ...
              <div className="space-y-3">
                {category.sections.map((section) => (
                  <div
                    key={section.id}
                    className="flex flex-col sm:flex-row sm:items-start sm:justify-between p-3 bg-gray-50 rounded-lg space-y-2 sm:space-y-0"
                  >
                    ...
                    <div className="flex items-center space-x-1 self-end sm:self-auto">
                      ...
                      <button
                        onClick={() =>
                          handleDeleteSection(section.id, section.sectionName)
                        }
                        className="p-1 text-gray-400 hover:text-red-500 transition-colors"
                      >
              ...
              {stats.categoryStats.map((category) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {category.totalViews.toLocaleString()} vistas totales
                    </p>
                  </div>
                  <div className="text-right">
  ...
  const renderAnalyticsPreview = () => {
    return <AnalyticsTab categories={categories} />;
  };
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleConfigChange("platformName", e.target.value)
                }
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {["MX", "CO", "EC", "SV", "GT", "HN", "DO", "PA"].map((country) => {
              const isEnabled =
                systemConfig.availableCountries.includes(country);
              return (
                <label
                  key={country}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newCountries = e.target.checked
                        ...
                        : systemConfig.availableCountries.filter(
                            (c) => c !== country,
                          );
            ...
            <div>
              <label className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) =>
                    handleConfigChange("publicContent", e.target.checked)
                  }
                  className="rounded text-[#124C45] focus:ring-[#124C45]"
                />
            ...
            <div>
              <label className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) =>
                    handleConfigChange("notifyUpdates", e.target.checked)
                  }
                  className="rounded text-[#124C45] focus:ring-[#124C45]"
                />
                <span className="text-sm">
                  Notificar actualizaciones a administradores
                </span>
              </label>
            </div>
            <div>
              <label className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) =>
                    handleConfigChange("showStats", e.target.checked)
                  }
                  className="rounded text-[#124C45] focus:ring-[#124C45]"
                />
                <span className="text-sm">
            ...
            <div>
              <label className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) =>
                    handleConfigChange("autoPublish", e.target.checked)
                  }
                  className="rounded text-[#124C45] focus:ring-[#124C45]"
                />
            ...
            <div>
              <label className="flex items-center space-x-2">
                ...
                <span className="text-sm text-red-600">
                  Modo de mantenimiento (oculta el sitio público)
                </span>
              </label>
            </div>
          </div>
